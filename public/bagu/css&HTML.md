...
1. HTML5 表单验证，原生提供了**内置验证属性**，常见的有：
   ...
   - 特点：
     ...
     - 不需要 JS 代码
     - 用户体验好，移动端也会弹出原生提示
2. 自定义验证：有些场景 HTML5 自带的验证无法满足，比如：- 复杂密码规则 - 异步验证（用户名是否已存在）- 联动验证（两个字段必须匹配）
   这时就需要**JavaScript 自定义验证**，常用方法：1. **oninput / onchange**：输入实时验证2. **onSubmit 拦截**：`form.addEventListener('submit', function(e) {...}`
...
**BEM** 是 **Block (块)、Element (元素)、Modifier (修饰符)** 的缩写，是一种用于组织 CSS 类名的命名规范。它旨在解决 CSS 全局作用域带来的命名冲突和样式依赖问题，使样式代码更具可读性和可维护性。示例：.card**title--large .card**image--dark
...
光标闪烁效果的核心是用 CSS 动画控制一个竖线元素的 `opacity` 或 `visibility` 在 0 和 1 之间切换。
...
1. **`step-end` 时间函数**：让动画在关键帧之间"跳跃"而非平滑过渡，模拟真实光标的"亮-灭"效果
2. **`infinite`**：无限循环
3. **频率**：通常 0.5s-1s 一个周期比较自然
...
**总结**：光标闪烁用 `animation` + `step-end` 实现跳跃式切换，可以用伪元素内容 `|` 或 `border-right` 模拟光标，配合 `steps()` 函数可实现完整的打字机效果。
