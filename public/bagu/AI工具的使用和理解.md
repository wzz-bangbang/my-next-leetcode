...
**RAG（Retrieval-Augmented Generation）= 检索增强生成**，核心是让 LLM 在生成回答前，先从知识库中检索相关内容作为上下文。

**回答思路**

1. **RAG 的基本流程**
   - 用户输入 → 向量化 → 检索知识库 → 拼接上下文 → LLM 生成回答
   - 前端通常不直接参与向量检索，但需要理解整个链路
...
3. **前端可能涉及的高级场景**
   - 文档上传和预览（用户上传 PDF/Word 作为知识库）
   - 知识库管理界面（增删改查文档）
   - 可视化检索结果和匹配度
   - Prompt 模板管理（让用户选择不同场景）

4. **与后端的协作边界**
   - 向量化、检索、Embedding 通常在后端/服务端
   - 前端负责交互体验，不直接处理向量计算
   - 需要与后端约定好接口格式（检索结果、引用来源等）
