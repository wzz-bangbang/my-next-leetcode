...
AI 应用（尤其是 LLM Chat 类）除了传统的 Web 性能指标（LCP、FCP、TTI）外，还需要关注一些 **AI 特有的体验和性能指标**：

**1. 首 Token 时间（Time to First Token, TTFT）**
- 从用户发送请求到收到第一个 token 的时间
- 这是用户感知"AI 开始响应"的关键点
- 类似于传统的 TTFB，但针对流式输出
- **参考值**：< 1s 体验较好，> 3s 用户会明显感到等待

**2. Token 生成速率（Tokens per Second, TPS）**
- 每秒输出的 token 数量
- 影响用户阅读体验，太慢会让用户等待，太快可能来不及阅读
- **参考值**：20-50 TPS 比较舒适

**3. 完整响应时间（Total Response Time）**
- 从发送请求到收到完整回复（包括 `[DONE]` 信号）的总时间
- 用于评估整体响应效率

**4. 流式渲染帧率（Streaming Render FPS）**
- 流式输出过程中页面的渲染帧率
- 如果大量 DOM 更新导致掉帧，用户体验会很差
- **监控方式**：`requestAnimationFrame` 计算实际帧间隔

**5. 中断率（Abort Rate）**
- 用户主动点击"停止生成"的比例
- 高中断率可能意味着响应太慢或内容不符合预期
...
**7. 上下文长度与耗时关系**
- 随着对话轮次增加，请求 payload 变大
- 监控 context length vs response time 的关系曲线
...
if (!metrics.firstTokenTime) {
  ...
  metrics.ttft = metrics.firstTokenTime - metrics.requestStart;
}

// 每个 chunk
metrics.tokenCount += chunkTokens;
metrics.lastTokenTime = Date.now();

// 结束时计算
const totalTime = metrics.lastTokenTime - metrics.requestStart;
const streamingTime = metrics.lastTokenTime - metrics.firstTokenTime;
const tps = metrics.tokenCount / (streamingTime / 1000);

// 上报
sendBeacon('/analytics', { ttft, tps, totalTime, aborted: false });
```

**总结**：AI 应用的性能监控核心在于 **TTFT（首 token 时间）** 和 **TPS（生成速率）**，它们直接影响用户对"AI 响应快不快"的感知。同时要监控流式渲染的帧率和用户行为指标（中断率、重试率），综合评估体验质量。