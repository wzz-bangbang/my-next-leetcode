...
export default function Header() {
  ...
  const navItems = [
    { href: '/bagu', label: '八股题', mobileLabel: '八股', Icon: BookOpenIcon, theme: 'bagu' },
    { href: '/code-editor', label: '刷题', mobileLabel: '刷题', Icon: RocketIcon, theme: 'code' },
  ];

  return (
    <header
      className="relative z-20 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between border-b border-gray-100 backdrop-blur-sm"
      style={{
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`btn-gradient-border btn-gradient-${item.theme} px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 ${
                  isActive ? 'active' : ''
                }`}
              >
                <span className="hidden sm:flex items-center gap-1.5">
                  <item.Icon size={iconSize.md} />
                  {item.label}
                </span>
                <span className="flex sm:hidden items-center gap-1">
                  <item.Icon size={iconSize.sm} />
                  {item.mobileLabel}
                </span>
              </Link>
            );
