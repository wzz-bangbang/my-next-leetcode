 ...
 * 从服务器加载题目状态
 */
export async function loadQuestionStatusFromServer(
  type: QuestionTypeKey,
): Promise<Record<number, QuestionStatus>> {
...
export async function setQuestionStatus(
  ...
  success: boolean;
  finalStatus: QuestionStatus;
  httpStatus?: number;
}> {
  const typeEnum = typeKeyToEnum[type];

  const { ok, status: httpStatus } = await questionStatusApi.saveQuestionStatus(
    questionId,
    typeEnum,
    status,
  );
